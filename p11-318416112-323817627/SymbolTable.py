class SymbolTable:
    ...
    def reset(self):
        for name in self.counters_dict:
            if name not in ['static', 'field']:
                self.counters_dict[name] = 0
    ...
    def define(self, name, type, kind):
        if kind in ['static', 'field']:
            self.class_scope[name] = (type, kind, self.counters_dict[name])
            self.counters_dict[name] += 1
        elif kind in ['arg', 'var']:
            self.current_scope[name] = (type, kind, self.counters_dict[name])
            self.counters_dict[name] += 1
